...
namespace lib60870.CS101
{
	...
	public class ASDU
	{
		...
		/// <summary>
		/// Инициализирует новый экземпляр <see cref="lib60870.CS101.ASDU"/> класса.
		/// </summary>
		/// <param name="parameters">параметры прикладного уровня, которые будут использоваться для кодирования / декодирования</param>
		/// <param name="cot">Причина передачи (COT)</param>
		/// <param name="isTest">Если значение <c>true</c>,  то у ASDU тестовый тип.</param>
		/// <param name="isNegative">Если установлено значение <c> true </c> - отрицательное подтверждение.</param>
		/// <param name="oa">Адрес отправителя (OA)</param>
		/// <param name="ca">Номер станции для ASDU (CA)</param>
		/// <param name="isSequence">Если значение <c>true</c>, то ASDU представляет собой последовательность информационных объектов.</param>
		...
		/// <summary>
		/// Получить тип идентификатора (TI).
		/// </summary>
		...
		/// <summary>
		/// Получить ил установить причину передачи (COT)
		/// </summary>
		...
		/// <summary>
		/// Gets the Номер станции of the ASDU (CA)
		/// </summary>
		...
		public byte[] RAW_1
		{
			get
			{
				Array.Resize<byte>(ref this.raw, 12);
				return raw;
			}
		}
