...
namespace lib60870.CS101
{
	public class QualityDescriptor
	{
		...
		{
			string strQuality = "";
			if (!Overflow & !Blocked & !Substituted & !NonTopical & !Invalid)
				return string.Format("GOOD");
			else
            {
				if (Overflow)
                {
					strQuality = strQuality + "[Overflow]";
				}
				if (Blocked)
				{
					strQuality = strQuality + "[Blocked]";
				}
				if (Substituted)
				{
					strQuality = strQuality + "[Substituted]";
				}
				if (NonTopical)
				{
					strQuality = strQuality + "[NonTopical]";
				}
				if (Invalid)
				{
					strQuality = strQuality + "[Invalid]";
				}
				return string.Format(strQuality);
			}		
		}
	}
