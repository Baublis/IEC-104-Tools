	...
	public class CP56Time2a
	{
		...
		public override string ToString()
		{
			if (DayOfMonth < 10 & Month < 10)
				return string.Format("{3}:{2}:{1}.{0}  0{5}:0{6}:{7}", Millisecond, Second, Minute, Hour, DayOfWeek, DayOfMonth, Month, Year, SummerTime, Invalid, Substituted);
			else if (DayOfMonth < 10 & Month >= 10)
				return string.Format("{3}:{2}:{1}.{0}  0{5}:{6}:{7}", Millisecond, Second, Minute, Hour, DayOfWeek, DayOfMonth, Month, Year, SummerTime, Invalid, Substituted);
			else if (DayOfMonth >= 10 & Month < 10)
				return string.Format("{3}:{2}:{1}.{0}  {5}:0{6}:{7}", Millisecond, Second, Minute, Hour, DayOfWeek, DayOfMonth, Month, Year, SummerTime, Invalid, Substituted);
			else
				return string.Format("{3}:{2}:{1}.{0}  {5}:{6}:{7}", Millisecond, Second, Minute, Hour, DayOfWeek, DayOfMonth, Month, Year, SummerTime, Invalid, Substituted);
		}
	}
}
